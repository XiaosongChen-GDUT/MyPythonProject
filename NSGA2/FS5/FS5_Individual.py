...
class Individual(object):
    ...
    def __init__(self):
        self.rank = None              # 秩，标志第几层
        self.crowding_distance = None # 拥挤距离
        self.domination_count = None  # 支配计数
        self.dominated_solutions = None  # 被支配的个体
        self.features = None          # 个体的特征（货道索引序列）
        self.objectives = None        # 个体的目标值
        self.allocated_nodes = []     # 缓存解码后的货位分配 [(node_id, sku, order), ...]
        self.unallocated = {}         # 缓存未分配的货位，key为sku，value为数量
        self.is_decoded = False       # 新增：是否已解码的标志
    ...
    #     # 检查 objectives 是否为 None
    #     if self.objectives is None or other_individual.objectives is None:
    #         raise ValueError(f"Objectives are None: self.objectives={self.objectives}, other_individual.objectives={other_individual.objectives}")
    #     and_condition = True
    #     or_condition = False
    #     # 逐一比较目标值
